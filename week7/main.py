...
from form import MemberForm, Message, deleteMessage, checkUsername, UpdateMemberName
...
async def delete_button(request: Request, message_id: int = Form(...)):
    ...
    if message.member_id != member_id:
        return RedirectResponse(url="/error?message=您沒有權限刪除此留言", status_code=302)
    
    deleteMessage(message_id)
    return RedirectResponse(url="/member", status_code=302) 

@app.get("/api/member", response_class=JSONResponse)
def search_member_name(request: Request, username: str):
    ...
    with Session(engine) as session:
        statement = select(MemberForm).where(MemberForm.username == username)
        result = session.exec(statement)
...
def update_member_name(request: Request, json_data: UpdateMemberName):
    member_id = request.session.get("id")
    ...
    try:
        with Session(engine) as session:
            member_data = session.get(MemberForm, int(member_id))
            new_member_data = json_data.model_dump(exclude_unset=True)
            member_data.sqlmodel_update(new_member_data)
